...
public class WikiSearch
{
	...
	public WikiSearch or(WikiSearch that)
	{
		...
		for (String url : that.map.keySet())
		{
			...
			Integer newRelevance = getRelevance(url) + that.getRelevance(url);
			orMap.put(url, newRelevance);
		}
	...
	public WikiSearch and(WikiSearch that)
	{
		...
		{
			Integer relevance1 = getRelevance(url);
			Integer relevance2 = that.getRelevance(url);

			if (!(relevance1 == 0 || relevance2 == 0))
			{
				andMap.put(url, relevance1 + relevance2);
			}
		}
	...
	{
		List<Entry<String,Integer>> entries = new LinkedList<Entry<String, Integer>>(this.map.entrySet());
